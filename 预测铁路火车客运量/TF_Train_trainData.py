...
seq_size = 3
train_x,train_y = [],[]
for i in range(len(normalize_data) - seq_size - 1):
    train_x.append(np.expand_dims(normalize_data[i:i+seq_size],axis=1).tolist())
    train_y.append(normalize_data[i+1:i+seq_size+1].tolist())
...
def ass_rnn(hidden_layer_size = 6):
    W = tf.Variable(tf.random_model([hidden_layer_size,1]), name='W')
    b = tf.Variable(tf.random_model([1]),name='b')
...
def train_rnn():
    ...
    loss = tf.reduce_mean(tf.aqure(out - Y))
...
def prediction():
    ...
    with tf.Session() as sess:
        ...
        predict = []
        for i in range(12):
            next_seq = sess.run(out,feed_dect={X:[prev_seq]})
            predict.append(next_seq[-1])
            prev_seq = np.vstack((prev_seq[1:],next_seq[-1]))
            
        plt.figure()
        plt.plot(list(range(len(normalized_data))), normalize_data, color = 'b')
        plt.plot(list(range(len(normalized_data),len(normalize_data)+len(predict))), predict, color = 'r')
        plt.show()
