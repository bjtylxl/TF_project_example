...
droup_keep_prob = tf.placeholder(tf.float32)
...
def neural_network():
    with tf.device('/cpu:0'), tf.name_scope("embedding"):
        ...
        embedded_chars = tf.nn.embedded_lookup(W, X)
    ...
    for i, filter_size in enumerate(filter_sizes):
        with tf.name_scope("conv-maxpool-%s" % filter_size):
            ...
            conv = tf.nn.max_pool(h, ksize=[1, input_size - filter_size+1, 1,1], strides=[1,1,1,1], padding='VALID')
    ...
    with tf.name_scope("dropout"):
        h_drop = tf.nn.dropout(h_pool_flat, droup_keep_prob)
        
    #output
    with tf.name_scope("output"):
        W = tf.get_variable("W", shape=[num_filters_total, num_classes], initializer = tf.contrib.layer.xavier_initializer())
...
def train_neural_network():
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entroy_with_logits(output,Y))
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            try:
                ...
                _, loss_ = sess.run([train_op, loss],feed_dict={X:batch_x, Y:batch_y,droup_keep_prob:0.5})
                print(loss_)
            ...
            if i % 10 == 0:
                ...
                accur = sess.run(accuracy,feed_dict={X:batch_x[0:50], Y:batch_y[0:50], droup_keep_prob:1.0})
