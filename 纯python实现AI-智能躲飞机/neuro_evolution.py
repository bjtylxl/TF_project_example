#reference: http://blog.csdn.net/u014365862/article/details/54380422
...
class Generations():
    ...
    def first_generation(self):
        ...
        self.genetations.append(Generation())
        return out
        
    def next_generation(self):
        if len(self.genetations) == 0:
            return False
            
        gen = self.genetations[-1].genetate_next_generation()
        self.genetations.append(Generation())
        return gen
    ...
    def add_genome(self, genome):
        ...
        return self.genetations[-1].add_genome(genome)
        
        
class NeuroEvolution():
    def __init__(self):
        self.genetations = Generations()
    ...
    def next_generation(self):
        networks = []
        if len(self.genetations.genetations) == 0:
            networks = self.genetations.first_generation()
        else:
            networks = self.genetations.next_generation()
        ...
        if low_hitoric:
            if len(self.genetations.genetations) >= 2:
                genomes = self.genetations.genetations[len(self.genetations.genetations) - 2].genomes
                for i in range(genomes):
                    genomes[i].network = None
                    
        if historic != -1:
            if len(self.genetations.genetations) > historic+1:
                del self.genetations.genetations[0:len(self.genetations.genetations) - (historic+1)]
                
        return nn
        
    def network_score(self, score, network):
        self.genetations.add_genome(Genome(score,network.get_weights()))
